...
Test(print_commands, check_help_command)
{
    client_t client;
    server_t server;
    int pipe_fds[2];
...
Test(print_commands, check_noop_command)
{
    client_t client;
    server_t server;
    int pipe_fds[2];
...
Test(print_commands, check_user_command)
{
    client_t client;
    server_t server;
    int pipe_fds[2];
...
Test(print_commands, check_pass_command)
{
    client_t client;
    server_t server;
    int pipe_fds[2];
    ...
    memcpy(client.name, "Anonymous", 9);
    check_command(&server, &client);
